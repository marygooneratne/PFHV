...
class RegressionModel:
    ...
    def clean_data(self):
        ...
        Returns:
            None
        '''
        self.market_value_df.dropna(axis=0,how='any')
        if self.verbose:
        ...
        if self.use_db:
            self.market_value_df['date'] = self.market_value_df['date'].apply(lambda x: x.year)
        else:
             self.market_value_df['date'] = self.market_value_df['date'].apply(lambda x: int(datetime.datetime.fromtimestamp(x/1e3).year))

        self.market_value_df.rename(columns={"date":"year"}, inplace=True)

...
REGIONAL_FILENAME = 'data/regional.csv'

def get_data(homes_filename=HOMES_FILENAME, history_filename=HISTORY_FILENAME, regional_filename=REGIONAL_FILENAME):
    db = Database()
    homes_df = db.homes_df
    history_df = db.history_df
    regional_df = db.macro_regional_df
    zipcode_df = db.zipcode_to_region_df
    return {"homes":homes_df, "history":history_df, "regional":regional_df, "zipcodes":zipcode_df}

def clean_data(homes_df, history_df, regional_df):
    '''
...
def append_data(row_prior, year_prior, regional_df, zipcode_df, zip_code):
    for _, row in zipcode_df.iterrows():
        if(row['zipcode'] == zip_code):
            region = row['region_id']
    for _, row in regional_df.iterrows():
        if(row['region'] == region and row['year'] == year_prior):
            row_prior = row_prior.append(row['housing_starts'])
            row_prior = row_prior.append(row['new_home_sales'])
    return row_prior

def build_predict_df(regional_df, zipcode_df, df, years=3):
    '''
    ...
    for _, row in df.iterrows():
        ...
        row_prior = append_data(row_prior, year_prior, regional_df, zipcode_df, row["zip_code"])
...
def predict(df):
    X = df[['pred_year', 'prev_value', 'zip_code', 'bedrooms', 'bathrooms', 'sq_ft', 'year_built','housing_starts','new_home_sales']]
    Y = df['pred_value']
...
if __name__ == "__main__":
    data = get_data()
    history_df = clean_data(data["homes"], data["history"],data["regional"])
    predict_df = build_predict_df(data["regional"], data["zipcodes"], history_df)
